...
- Use tool-based file displays (e.g., `view_file` diffs) instead of inline content.
- Provide concise summaries with references to modified files.
- Avoid showing code snippets in responses; link to files instead.
*   **Agent Persona**: [`.github/agents/z-sandbox-agent.md`](.github/agents/z-sandbox-agent.md)
## Ask Big D (do this, not that)
...
## Grok Multi-Agent Tooling
Grok must collaborate with the local CLIs below for planning, reasoning, and brainstorming. Every invocation is conversation-only: do not request file edits, shell commands, or repo mutations from these tools. Commands already disable approvals where possible and keep scope to this workspace.
### Grok CLI — self-reflective loop
- Command: `grok -p "" --model grok-4-latest --max-tool-rounds 0`
- Use for: meta planning, sanity-checking strategies, or rehearsing explanations when you need Grok’s own model without tool execution. Tool rounds are disabled, guaranteeing zero shell/file actions.
- Workflow: Use `-p` for single one-shot prompts; omit for interactive sessions; rely on `GROK_API_KEY`, `GROK_MODEL`, etc., already configured in the environment.
### Copilot — implementation sounding board
- Command: `bash -lc 'copilot -p ""'`
- Use for: discussing implementation variants, GitHub-centric workflows, or summarizing repository context from Copilot’s perspective. Auto-approvals avoid interaction stalls, but explicitly remind Copilot to stay advisory (no write/apply steps) since this pipeline is for dialogue only.
- Workflow: launch interactive sessions or run `-p "<prompt>"` for targeted questions. Do not add directories or disable path checks unless separately authorized.
### Codex — automation mirror
- Command: `bash -lc 'codex --sandbox workspace-write --ask-for-approval never "$@"'`
- Use for: validating automation ideas, exploring how Codex might execute plans locally, or simulating Codex responses when coordinating multi-agent workflows. Approvals are disabled, but the workspace-write sandbox keeps scope limited to this repo.
- Workflow: pass prompts directly or leverage `codex exec -p "<prompt>"` for single responses. Do not request destructive git commands or actual file edits without an explicit human directive.
### Collaboration Pattern
- Default to Grok’s internal reasoning; invoke external agents when additional perspectives or confirmation loops are needed.
- State in prompts that the agents are advisory only, so they know to avoid editing actions despite auto-approval settings.
- Cross-reference insights between agents before modifying code or documentation, and record which agent informed any plan when summarizing decisions to humans.
