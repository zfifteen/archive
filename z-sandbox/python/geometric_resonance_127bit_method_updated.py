...
def resonance_candidates(N, num_samples=801, k_lo=0.25, k_hi=0.45, m_span=180, J=6, progress=False):
    ...
    J = config['J']
    threshold_multiplier = config.get('threshold', 0.92)
    
    threshold = (2 * J + 1) * mpf(str(threshold_multiplier))
    b = mpf(str(config['bias']))
    sqrtN = sqrt(N)
    cands = set()    
...
def factor_by_geometric_resonance(N_int, config=None):
    ...
    cands = resonance_candidates(N, config, progress=True)    
...
def main():
    ...
    config = {
        ...
        output_dir.mkdir(parents=True, exist_ok=True)    factors = factor_by_geometric_resonance(N_int, config)
