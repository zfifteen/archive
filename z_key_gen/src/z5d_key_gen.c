...
static void print_usage(const char *progname) {
    ...
    printf("  --show-hardware        Display hardware information (macOS only)\n");
...
static int parse_args(int argc, char **argv, config_t *cfg) {
    static struct option long_options[] = {
        ...
        {"show-hardware", no_argument, 0, 's'},
        {"help", no_argument, 0, 'h'},
        {0, 0, 0, 0}
    };

    int c;
    while ((c = getopt_long(argc, argv, "b:e:v:g:k:p:P:Q:dqsh", long_options, NULL)) != -1) {
        switch (c) {
            case 'b':
