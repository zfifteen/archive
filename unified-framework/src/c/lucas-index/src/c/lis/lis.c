...
/* PoC: Fast-doubling Fibonacci modulo m (utility for P=1,Q=-1). */
...
/* PoC: Selfridge/Kronecker-5 style Lucas/Fibonacci probable-prime filter for P=1,Q=-1. */
static int lucas_frobenius_filter(uint64_t n, const lis_config_t* cfg) {
    ...
    if (!(cfg && cfg->P == 1 && cfg->Q == -1)) {
        /* Only P=1,Q=-1 supported in this PoC. */
        return 1; /* Skip filter if different parameters. */
    }
