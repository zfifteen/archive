...
def sample_semiprimes_wide(Nmax, prime_pool, samples, rng):
    # wide: tiny p, large q
    small_primes = [p for p in prime_pool if p <= 200]
    candidates = []
    while len(candidates) < samples:
        p = rng.choice(small_primes)
        qmax = Nmax // p
        # choose q as a random prime below qmax, biased high
        hi_band = max(3, int(qmax * 0.9))
        hi_band = min(hi_band, prime_pool[-1])
        qs = [q for q in prime_pool if q <= qmax and q >= hi_band]
        if not qs:
            qs = [q for q in prime_pool if q <= qmax]
            if not qs: 
                continue
        q = rng.choice(qs)
        if p*q < Nmax:
...
def main():
    ...
    # choose sampler
    semis = sample_semiprimes_wide(Nmax, prime_pool, SAMPLES, rng)
