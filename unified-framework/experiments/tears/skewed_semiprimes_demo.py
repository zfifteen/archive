...
def sample_semiprimes_skewed(Nmax, prime_pool, samples, rng):
    # skewed: p ~ sqrt(Nmax)/3, q ~ 3*sqrt(Nmax)
    R = int(Nmax**0.5)
    band_p = (max(2, R//5), max(3, R//2))
    band_q = (R*2, R*4)
    Ps = [p for p in prime_pool if band_p[0] <= p <= band_p[1]]
    Qs = [q for q in prime_pool if band_q[0] <= q <= band_q[1]]
    if not Ps: Ps = [p for p in prime_pool if p <= R]
    if not Qs: Qs = [q for q in prime_pool if q <= Nmax//max(2, band_p[0])]
    candidates = []
    while len(candidates) < samples:
        p = rng.choice(Ps); q = rng.choice(Qs)
        if p*q < Nmax:
...
def main():
    ...
    # choose sampler
    semis = sample_semiprimes_skewed(Nmax, prime_pool, SAMPLES, rng)
