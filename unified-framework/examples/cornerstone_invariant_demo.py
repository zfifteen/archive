...
def demo_validation_checks():
    ...
    for (check_name, description), key in zip(checks, 
                                               ['universality_check', 'consistency_check',
                                                'reproducibility_check', 'symmetry_check',
                                                'precision_check']):
        passed = validation[key]
        status = "✅ PASS" if passed else "❌ FAIL"
        print(f"{check_name:20s}: {status}")
        print(f"{'':20s}  {description}")
        
        if not passed:
            error_key = key.replace('_check', '_error')
            if error_key in validation:
                print(f"{'':20s}  Error: {validation[error_key]}")
        print()
    
    overall_status = "✅ ALL CHECKS PASSED" if validation['overall_pass'] else "❌ SOME CHECKS FAILED"
    print("-" * 80)
