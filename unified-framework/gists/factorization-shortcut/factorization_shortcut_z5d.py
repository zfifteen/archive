...
PRIME_COUNT_SAFETY_FACTOR = 1.3  # Safety factor for prime count estimation (π(x) ≈ x/ln(x) * factor)
...
def z5d_generate_prime(k: int) -> int:
    ...
    Args:
        k: Prime index (k >= 2). Note: In some indexing schemes, k=1 corresponds to the first prime (2),
            but in this implementation, k=2 yields the second prime (3), k=3 yields 5, etc.

    Returns:
        The k-th prime number

    Raises:
        ValueError: If k < 2. This is because k=1 would correspond to the first prime (2) in some schemes,
            but this implementation starts at k=2 for the second prime (3).
        RuntimeError: If z5d_prime_gen not found or fails
    """
    if k < 2:
        raise ValueError(f"k must be >= 2 (k=1 would correspond to the first prime 2 in some schemes, but this implementation starts at k=2 for the second prime 3); got {k}")
...
def z5d_primes(limit: int, verbose: bool = False) -> List[int]:
    ...
    if limit < 100:
        ...
        approx_count = int( PRIME_COUNT_SAFETY_FACTOR * limit  / math.log(limit))  # Safety factor for prime count estimation
